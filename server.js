...
const serverMeta = require('./src/serverMeta');
const { startDeadlineReminderScheduler } = require('./src/utils/deadlineReminderScheduler');
...
async function init() {
  ...
  app.listen(PORT, HOST, () => {
    ...
    // record server start timestamp for uptime reporting
    try { serverMeta.setStart(Date.now()); } catch (e) { /* noop */ }
    try { startDeadlineReminderScheduler(); } catch (e) { /* noop */ }
  });
}
