...
const allowedFields = ["group_id","name","created_at"];
...
// List
router.get('/generated/group/', requireAdmin, async (req, res) => {
  try {
...
// Get by PK
router.get('/generated/group/:id', requireAdmin, async (req, res) => {
  try {
...
// Create
router.post('/generated/group/', requireAdmin, express.json(), async (req, res) => {
  try {
...
// Update
router.put('/generated/group/:id', requireAdmin, express.json(), async (req, res) => {
  try {
...
// Delete
router.delete('/generated/group/:id', requireAdmin, async (req, res) => {
  try {
    const item = await Model.findByPk(req.params.id);
    if (!item) return res.status(404).json({ error: 'Not found' });
    await item.destroy();
