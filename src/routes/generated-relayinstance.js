...
// List
router.get('/generated/relayinstance/', requireAdmin, async (req, res) => {
  try {
...
// Get by PK
router.get('/generated/relayinstance/:id', requireAdmin, async (req, res) => {
  try {
...
// Create
router.post('/generated/relayinstance/', requireAdmin, express.json(), async (req, res) => {
  try {
...
// Update
router.put('/generated/relayinstance/:id', requireAdmin, express.json(), async (req, res) => {
  try {
...
// Delete
router.delete('/generated/relayinstance/:id', requireAdmin, async (req, res) => {
  try {
    const item = await Model.findByPk(req.params.id);
    if (!item) return res.status(404).json({ error: 'Not found' });
    await item.destroy();
